/**
 * ============================================================
 * rtc.h — DS1307 real-time clock over I2C (Arduino Uno)
 * ============================================================
 *
 * Shared module contract (identical on all four platforms):
 *   init / get_time / set_time / timestamp
 *
 * rtc_timestamp() is the one function in this module with no hardware
 * in it, so it is implemented for real on every port. Every log line
 * in main.cpp is prefixed with it, which is what keeps the entry
 * points readable across platforms.
 *
 ...
 * 3.3V. See their rtc.h / rtc.py for the workarounds.
 * ============================================================
 */
...
/* ── I2C pins — fixed by the ATmega328P's TWI hardware ────────────
 * A4 = SDA, A5 = SCL. Not configurable, unlike the ESP32.
 * 4.7k pull-ups to 5V; most DS1307 breakout boards fit them already.
 */
constexpr uint8_t RTC_SDA_PIN = 18;  // A4
constexpr uint8_t RTC_SCL_PIN = 19;  // A5
 ...
 * Falls back to "[---------- --:--:--] " if the clock cannot be read,
 * so a dead RTC never silently produces a plausible-looking timestamp
 * on a security log.
 *
