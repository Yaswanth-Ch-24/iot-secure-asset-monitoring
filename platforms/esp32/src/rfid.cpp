/**
 * ============================================================
 * rfid.cpp — RC522 MIFARE reader over SPI (ESP32)
 * ============================================================
...
bool rfid_init() {
  ...
  // TODO: SPI.begin(RFID_SCK_PIN, RFID_MISO_PIN, RFID_MOSI_PIN, RFID_CS_PIN);
  // TODO: SPI.setFrequency(RFID_SPI_HZ);
  // TODO: SPI.setDataMode(SPI_MODE0);   // CPOL 0, CPHA 0 — as on SPI1
   ...
   */
  Serial.println("TODO: rfid_init() -> bring up VSPI + RC522");
  return false;
}
...
bool rfid_read_uid(RfidUid* uid) {
   ...
   *       first four). The STM32 driver skips the BCC check — do NOT
   *       skip it here: an unverified UID on an access-control path
   *       means a corrupted read can match a real card.
   */
  (void)PICC_SEL_CL1;
  return false;
}
